                  ...
                  <table id="datatable" class="table table-striped table-bordered w-100">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="text-center"><?=$data->offer_name?></td>
                        <td class="text-center"><?=$data->offer_description?></td>
                        <td class="text-center">
                          <img class="img-responsive" src="<?=base_url('assets/admin/uploads/Offer/').$data->offer_image?>" style="height:50px">
                        </td>                    
                       <td class="text-center d-flex justify-content-center">
                        <form method="post" action="<?=base_url('admin/Offers/editoffer')?>">  
                          <input type="hidden" name="offer_id" value="<?=$data->offer_id?>" class="form-control">
                          <button type="submit" class="btn btn-primary">Edit</button>
                        </form>
                        <a class="btn btn-danger" id="offerdelete" offer_id="<?=$data->offer_id?>" image_name="<?=$data->offer_image?>" href="javascript:void(0)">Delete</a>
                      </td>
                      </tr>
                    <?php
                    $count++;
...
<script type="text/javascript">
  $(document).on("click","#offerdelete",function(){
    ...
    {
     var offer_id =ele.attr("offer_id");
      var image_name=ele.attr("image_name");
     $.ajax({
            url:"<?=base_url('admin/Offers/deleteoffer')?>",
             type:"post",
             data:{offer_id:offer_id,image_name:image_name},
             cache:false,
